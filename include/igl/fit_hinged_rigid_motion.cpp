...
)
{
	...
	Eigen::PlainObjectBase<DerivedV> cv1 = v1;
	Eigen::PlainObjectBase<DerivedV> cd1 = d1;
	Eigen::PlainObjectBase<DerivedV> cv2 = v2;
	Eigen::PlainObjectBase<DerivedV> cd2 = d2;
	...
	while (iter < max_iter) {
		...
		fit_rigid_motion(cv1, cd1, weight_1, R1, t1);
		fit_rigid_motion(cv2, cd2, weight_2, R2, t2);
		...
		Matrix3d R1tR1 = R1 * R1_trans;
		RowVector3d Q1te1 = Vector3d::Zero();
		RowVector3d R1te1 = Vector3d::Zero();
		...
		Matrix3d R2tR2 = R2 * R2_trans;
		RowVector3d Q2te2 = Vector3d::Zero();
		RowVector3d R2te2 = Vector3d::Zero();
		...
		cv1 = v1 * R1 + t1.replicate(v1.rows(), 1);
		cv2 = v2 * R2 + t2.replicate(v2.rows(), 1);
